...
Sweeper is a tool for generating 3D meshes from a set of cross-sections and a sweep-line, developed for the 2IMV10 Visual Computing Project course at the TU/e. It was developed together with Boris Rokanov and Georgi Kostov.

The goal of tihs website is to allow people that have no experience with 3D modeling to easily create simple objects, while also allowing the creation of a variety of more advanced objects.
The following video presents the website, and explains how it can be used:
...
It also used the following less important libraries, which are still worth mentioning:
...
This project contains several interesting aspects which are discussed in the sections below.

### Sweep algorithm

The sweep algorithm has been developed as an independent piece of code without any external dependencies. It takes the set of cross-sections as an input together with the sweep-line and some additional settings, and outputs a 3D mesh. This mesh is defined as a set of 3D points and triplets of (indices of) these points that form the triangles of the mesh.

The exact type definition of the sweep function is described by the following typings:
...
This specification allows the algorithm to simply retrieve a number of nodes along the sweep-line, where cross-sections should be placed. As well as approximations of the specified cross-sections in the form of polygons with the specified number of points.

The algorithm derives intermediate cross-sections to place at all the sweep-line nodes by linearly interpolating the points of the user specified cross-sections. The result of this can be seen in the folowwing video:
TODO:
...
The essence of this matrix rotation can be found in [transformRotationMatrix.ts](https://github.com/TarVK/3D-sweep-object/blob/595819db9c23aa402ff360587c8101360a2eb6f0/src/sweepOperation/core/transformRotationMatrix.ts#L13)
The main entry point of the algorithm can be found in [createSweepObject.ts](https://github.com/TarVK/3D-sweep-object/blob/main/src/sweepOperation/createSweepObject.ts)

### UI design

A challenge in this project was to keep the UI clean and simple, while still keeping it powerful enough to create numerous shapes. In addition we had to make sure that this simplicity does not ruin usability, since advanced features may be harder to get familiar with, but often make software easier/quicker to use in the long run.
...
One important goal of the project was to show the user a live view of the final sweep object, such that any change is immediately reflected in the mesh. To achieve this, a data model of the sweep object specification was made. This model uses my own state management library [Model-React](https://github.com/TarVK/model-react#readme). This allows both the sweep-line view and the cross-section view to subscribe to changes in the model, allowing them to instantly rerender when needed.
The mesh itself is also defined in terms of the specificationss, such that the mesh is instantly recalculated when changes occur.

Keeping this model separate from the views keeps this aspect of the code clean, such that we don't have messy dependencies between different components where callback props have to be passed around.
This also made it easy to write code for exporting the model to a plain JSON format. This JSON format can also be imported to a new model, and performs checks on the format and creates user friendly error messages if the provided text does not adhere to the format.
