...
Lambda calculus is a formal modal of computation. Alonzo Church came up with this model in the 1930s in order to define what constitutes a computation. He showed his model of computation to be equally powerful as [Turing Machines](https://en.wikipedia.org/wiki/Turing_machine), a competing model of computation which I will discuss another time.
...
```js
console.log(readBool(True(True)(False))); // true
console.log(readBool(False(True)(False))); // false
console.log(readBool(True(False)(True))); // false
console.log(readBool(False(False)(True))); // true
```
...
div = n=>m=>F(r=>n=>(s=>lazyIf(isZero(s))(v=>V0)(v=>add1(r(s))))(sub(n)(m)))(add1(n))
